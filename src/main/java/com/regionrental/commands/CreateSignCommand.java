...
public class CreateSignCommand implements CommandExecutor {
    ...
    public CreateSignCommand(RegionRental plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("regionrental.admin.createsign")) {
            player.sendMessage(plugin.getConfigManager().getMessage("no-permission"));
            return true;
        }
        
        if (args.length < 1) {
            player.sendMessage(ChatColor.RED + "Usage: /rrcreatesign <region>");
            return true;
        }
