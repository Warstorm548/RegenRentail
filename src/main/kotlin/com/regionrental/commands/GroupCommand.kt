 ...
 */
class GroupCommand(private val plugin: RegionRental) : CommandExecutor, TabCompleter {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        sender.checkPermission("regionrental.admin.group", "${ChatColor.RED}You don't have permission to use this command.")
            ?: return true
    ...
    private fun promptForRegions(sender: CommandSender, groupName: String, action: String) {
        val player = sender as? Player ?: run {
            ...
            val usage = when (action) {
                "create" -> "Usage: /rrgroup create $groupName region1,region2,world:region3"
                "add" -> "Usage: /rrgroup edit $groupName add region1,region2"
                "remove" -> "Usage: /rrgroup edit $groupName remove region1,region2"
                else -> ""
            }
    ...
    override fun onTabComplete(sender: CommandSender, command: Command, alias: String, args: Array<String>): List<String> {
        if (!sender.hasPermission("regionrental.admin.group")) return emptyList()
