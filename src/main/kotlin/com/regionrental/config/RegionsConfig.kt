...
class RegionsConfig(private val plugin: RegionRental) {
    ...
    fun verifyAndRepairRegions() {
        ...
        if (orphanedConfigs.isNotEmpty()) {
            ...
            plugin.logger.info("These configs are safe but may be unused. Use /rroverride remove <region> to clean up.")
        ...
        if (usingDefaults > 0 && plugin.configManager?.isDebug == true) {
            ...
            plugin.logger.info("Use /rroverride commands to set custom values for specific regions")
        }
    }
